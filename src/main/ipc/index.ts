...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle(
    CH.PROFILE_UPDATE,
    (_e, userId: string, updates: Parameters<typeof updateProfile>[1]) =>
  ...
  ipcMain.handle(CH.FRIEND_ADD, (_e, userId: string, friendId: string) =>
  ...
  ipcMain.handle(
    CH.FRIEND_REQUEST_SEND,
    (_e, requesterId: string, receiverId: string) =>
      sendFriendRequest(requesterId, receiverId),
  );
  ipcMain.handle(
    CH.FRIEND_REQUEST_ACCEPT,
    (_e, userId: string, requestId: string) =>
      acceptFriendRequest(userId, requestId),
  );
  ipcMain.handle(
    CH.FRIEND_NUDGE_SEND,
    (_e, fromUserId: string, toUserId: string, fromName: string) => {
      const payload = {
        fromUserId,
        toUserId,
        fromName,
  ...
  ipcMain.handle(CH.COHORT_CREATE, (_e, userId: string, name: string) =>
    createCohort(userId, name),
  );
  ipcMain.handle(CH.COHORT_JOIN, (_e, userId: string, inviteCode: string) =>
    joinCohort(userId, inviteCode),
  );
  ipcMain.handle(CH.COHORT_SHARED_PROFILES, (_e, userId: string) =>
    getSharedCohortProfiles(userId),
  );
  ipcMain.handle(CH.COHORT_MEMBERS, (_e, cohortId: string) =>
    getCohortMembers(cohortId),
  );
  ipcMain.handle(CH.COHORT_LEAVE, (_e, userId: string, cohortId: string) =>
  ...
  ipcMain.handle(CH.SESSION_HISTORY, (_e, userId: string) =>
    getSessionHistory(userId),
  );
  ipcMain.handle(CH.AGENT_QUERY, async (_e, request: AgentRequest) => {
    console.log("[agent] IPC query received:", request);
    try {
      const result = await queryAgent(request);
      console.log("[agent] IPC query result:", JSON.stringify(result, null, 2));
      return result;
    } catch (error) {
      console.error("[agent] IPC query error:", error);
      return { text: "agent unavailable", error: String(error) };
    }
  });
  ...
  ipcMain.handle(
    CH.MQTT_PUBLISH_COMMAND,
    (_e, userId: string, command: Record<string, unknown>) => {
  ...
  ipcMain.handle(
    CH.HW_SIMULATE,
    (_e, userId: string, payload: Record<string, unknown>) =>
  ...
  // Shell/auth windows
  ipcMain.handle(CH.OPEN_EXTERNAL, (_e, url: string) =>
    shell.openExternal(url),
  );
  ipcMain.handle(CH.OPEN_AUTH_WINDOW, (event, url: string) =>
  ...
  ipcMain.handle("get-config", () => ({
    ...
    LOCAL_VLM_URL:
      (import.meta.env.LOCAL_VLM_URL as string) ??
  ...
  ipcMain.handle(
    ...
    async (
      _e,
      {
        ...
        workflowGroup,
      }: { plannedDurationMinutes: number; workflowGroup: string },
    ) => {
  ...
  ipcMain.handle(
    ...
    async (
      _e,
      {
        ...
        endpoint,
        model,
      }: { imageDataUrl: string; endpoint?: string; model?: string },
    ) => {
      ...
      const ep =
        ...
        "http://127.0.0.1:11434/api/chat";
      const m =
        model ?? (import.meta.env.LOCAL_VLM_MODEL as string) ?? "moondream";
      ...
      const text = String(data.message?.content ?? data.response ?? "")
        .toLowerCase()
        .trim();
      if (text.includes("distracted") || text.includes("distraction"))
        return "distracted";
      if (text.includes("admin")) return "admin";
      if (text.includes("deep") || text.includes("productive"))
        return "deep_work";
      return "distracted";
    },
  );
...
//         // sandbox: true, // Disabled to allow OAuth
